...
function publicPacketUrl(inputUrl) {
  const url = new URL(inputUrl);
  for (const key of Array.from(url.searchParams.keys())) {
    if (isSensitiveParam(key) || isUsernameParam(key)) {
...
async function handleSkillCall(params = {}) {
  try {
    ...
    const query = buildPluginQuery(params);
    const startedAt = new Date().toISOString();
...
module.exports = {
  handleSkillCall,
  executePacketTask: handleSkillCall,
  resolveQuery,
  normalizeCriteria,
  validateCriteria,
  buildUrls,

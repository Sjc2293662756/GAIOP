...
async function executePacketTask(query = {}) {
...
function buildUrls({ host, criteria, downloadType, showFullUrls }) {
  if (downloadType === 'DownServlet' || criteria.instanceId) {
    ...
    return {
      download: showFullUrls ? url.toString() : maskUrl(url.toString()),
  ...
  return {
    preview: showFullUrls ? preview : maskUrl(preview),
    download: showFullUrls ? download : maskUrl(download),
    previewRaw: preview,
...
function httpRequest(inputUrl, options = {}) {
  const url = new URL(inputUrl);
  applyRuntimeQueryAuthentication(url);
...
function applyRuntimeQueryAuthentication(url) {
  const username = process.env.NETINSIDE_USERNAME;
  const password = process.env.NETINSIDE_PASSWORD;
  if (!username || !password || hasAuthQueryParams(url.searchParams)) return;
...
function buildPluginQuery(params = {}) {
  const input = isPlainObject(params) ? params : {};
  const packetQuery = isPlainObject(input.packetQuery) ? input.packetQuery : {};
  const query = {
    ...packetQuery,
    ...input
  };
  delete query.packetQuery;

  const packetCriteria = isPlainObject(packetQuery.criteria) ? packetQuery.criteria : {};
  const inputCriteria = isPlainObject(input.criteria) ? input.criteria : {};
  if (Object.keys(packetCriteria).length > 0 || Object.keys(inputCriteria).length > 0) {
    query.criteria = { ...packetCriteria, ...inputCriteria };
  }

  const packetAnalysis = isPlainObject(packetQuery.analysis) ? packetQuery.analysis : {};
  const inputAnalysis = isPlainObject(input.analysis) ? input.analysis : {};
  if (Object.keys(packetAnalysis).length > 0 || Object.keys(inputAnalysis).length > 0) {
    query.analysis = { ...packetAnalysis, ...inputAnalysis };
  }

  const packetFilePolicy = isPlainObject(packetQuery.filePolicy) ? packetQuery.filePolicy : {};
  const inputFilePolicy = isPlainObject(input.filePolicy) ? input.filePolicy : {};
  if (Object.keys(packetFilePolicy).length > 0 || Object.keys(inputFilePolicy).length > 0) {
    query.filePolicy = { ...packetFilePolicy, ...inputFilePolicy };
  }

  return query;
}

function packetRuntimeFailure(error, includeStack = false) {
  const extra = includeStack && error?.stack ? { stack: error.stack } : {};
  return failureResult('PACKET_RUNTIME_ERROR', error?.message || String(error), extra);
}
...
async function main() {
  ...
  try {
    writeJson(await executePacketTask(query));
  } catch (error) {
    writeJson(packetRuntimeFailure(error, true));
    process.exitCode = 1;
  }
}

/**
 * The plugin calls the packet Skill in-process with ordinary JavaScript objects.
 */
async function handleSkillCall(params = {}) {
  try {
    loadRuntimeEnv();
    return await executePacketTask(buildPluginQuery(params));
  } catch (error) {
    return packetRuntimeFailure(error);
  }
}

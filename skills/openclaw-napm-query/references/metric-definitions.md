...
- 页面性能指标（Page Performance Metrics）

解释型问题优先基于本文件回答；查询型问题在解析 metric code 时，应结合运行时 `metrics` 元数据服务进一步确认。`metrics` 服务可返回设备支持的指标列表与代码。
...
- 页面时间（Page Time）：从一个页面视图中第一个到最后一个 HTTP 响应的平均时间。
- 常见 code：
  - `PGTME`：页面延时
  - `PGTMS`：页面延时（客户端）
  - `PGTMC`：页面延时（服务器）
...
- 含义：TCP 包重传占观测包的百分比，高值通常表示网络丢包或严重传输问题。
- 常见 code：`PLI`、`PLO`。
...
- 含义：仅统计有效 TCP 负载的数据传输速率，不含重传与无数据包。
- 常见 code：
  - `GPI`
  - `GPO`
...
- TCP Turn 指标标为 `Client` / `Server`

解释时不要混淆。

### 6.2 Inbound / Outbound 的含义与 group 有关

对于利用率指标，Inbound / Outbound 表示相对 group 的流向；对某些 group（如 Application、Total Traffic、VLAN、Mon Interface Group），方向依赖 Internal Address List。
...
可通过 `metrics` 与 `metricsForGroup` 服务确认。

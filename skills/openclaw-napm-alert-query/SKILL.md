...
**格式规则**：
- 仅展示告警总数大于 0 的类别；类别序号按实际展示顺序连续编号
- 当告警总数为 0 时，不展示任何类别标题，只显示严重程度统计和"本时间范围内未查询到告警事件。"
- 类别汇总行（序号、类别、总数和严重程度统计）统一使用 `# **...**` 兼容标题；`#` 保证企业微信等有限 Markdown 通道提升行级层级，`**...**` 保留标准 Markdown 加粗语义
- 同类下按"告警名称 + 严重级别"分组聚合，组间 `---` 分隔
- 分类汇总行由 `AlertDisplayFormatService` 生成；插件仅透传 `displayText`，缺少该字段时复用同一 formatter，不得自行改写标题

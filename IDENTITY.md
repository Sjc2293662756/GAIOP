...
- 故障诊断分析（B/S 业务 HTTP 错误分析、B/S 页面性能延时分解、C/S 应用慢分析、网络慢分析）
- 故障诊断报告（BS业务故障/BS页面性能/CS应用故障/网络故障，自动检测 flowType）
- 数据包分析（下载、预览、业务页面定位）
- 企业微信中的运维问答和排障辅助
...
- 故障诊断流程：bs_app_slow（业务HTTP错误）、bs_page_perf（页面性能延时分解）、cs_app_slow（应用慢分析）、network_slow（网络慢分析）
- 报告类型：巡检报告(inspection)、故障诊断报告(diagnostic，含BS业务/BS页面性能/CS应用/网络)、综述报告(summary)

 ...
 */
function computeOverallStatus(alertSummary = {}, trafficTrend = {}, businessSummary = {}) {
...
class SummaryService {
  ...
  _planWebApplicationTarget(scope, start, end, granularity) {
    ...
    return [
      { label: 'alertsSummary', fn: () => this.client.getAlertsSummary(start, end) },
      { label: 'alertsTimeline', fn: () => this.client.getAlertsTimeline(start, end) },
      { label: 'trafficTrend', fn: () => this.client.getTimeValues(start, end, 'TPIO,TPI,TPO,PGSLPCT,PGTME,PGHTTP400,PGHTTP500', [{ type: 'WebApplication', argument: groupArg }], granularity) },
      { label: 'drillDown', fn: () => this.client.getTopValues(start, end, 'TPIO,TPI,TPO', 'TPIO', 10, [{ type: 'WebApplication', argument: groupArg }, { type: 'ClientIPs' }, { type: 'IPAddress' }]) },
      { label: 'slowAccess', fn: () => this.client.getTopValues(start, end, 'PGSLPCT,PGNSLPGE,PGTME', 'PGSLPCT', 10, [{ type: 'WebApplication', argument: groupArg }]) },
      { label: 'httpErrors', fn: () => this.client.getTopValues(start, end, 'PGHTTP400,PGHTTP500', 'PGHTTP500', 10, [{ type: 'WebApplication', argument: groupArg }]) },
      { label: 'applianceInfo', fn: () => this.client.getApplianceInfo() }
    ];
  }
  ...
  aggregate(rawData = {}, scope = {}, timeRange = {}) {
    ...
    const summary = {
      ...
      trafficSummary: this._aggregateTraffic(rawData, scopeType),
      businessSummary: this._aggregateBusiness(rawData, scopeType),
      conclusion: null,
      recommendations: []
    };
    return summary;
  ...
  _aggregateAlerts(rawData = {}, scope = {}) {
    ...
    // For scoped with target: filter by object
    if (scopeType !== 'global' && target.groupType && target.groupArgument) {
      const filtered = (alertSummary.unresolvedAlerts || []).filter(
  ...
  _aggregateTraffic(rawData = {}, scopeType = 'global') {
    ...
    if (scopeType !== 'alert' && rawData.trafficTrend) {
      ...
      // Collect all numeric values for stats (use TPIO as primary)
      ...
      // Convert parallel arrays to points[] format for chart rendering
    ...
    // Top Groups (businessGroup overview)
    if (scopeType === 'businessGroup' && rawData.topGroups) {
      trafficSummary.topApps = normalizeTopValues(rawData.topGroups, 'BusinessGroup');
    }

    // Drill-down (scoped with target)
    if (rawData.drillDown) {
      const targetGroupType = scopeType === 'webApplication' ? 'IPAddress' : 'IPAddress';
      trafficSummary.drillDown = {
        groupType: targetGroupType,
        label: '下级维度',
        items: normalizeTopValues(rawData.drillDown, targetGroupType)
      };
    }

    // App connections (application overview)
    ...
    // App failures (application overview)
    ...
    // App quality (application overview: packet loss + RTT)
  ...
  _aggregateBusiness(rawData = {}, scopeType = 'global') {
    const businessSummary = { slowAccess: [], httpErrors: [], pageTraffic: [], pageViews: [] };
    ...
    // Page traffic (PGBYTI/PGBYTO — only for webApplication overview)
    ...
    // Page views (PGNPGE — only for webApplication overview)

...
async function executeAlertQuery(payload = {}, options = {}) {
  // 🔴 2026-07-27 修复: 从 prompt 解析相对时间必须在校验之前完成。
  // 否则 AI 传入 criteria:{} 时，即使 prompt 明确包含"最近三小时"，
  // validateAlertQuery 也会因缺少 start/end 而失败。
  ...
  if (promptTimeRange) {
    // 兼容两种参数格式：嵌套 { alertQuery: { criteria } } 和扁平 { criteria }

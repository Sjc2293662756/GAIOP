  ...
  register(api) {
    ...
    registerNapmHook(
      ...
      (event, ctx) => {
        ...
        // 🔴 2026-07-27 修复: napm-alert-query 的默认时间逻辑独立提取到
        // try-catch 之外，不依赖 timeResolver require 是否成功。
        // 防止 require 偶发失败时校验缺少 start/end → 误导性"0条告警"。
        if (toolName === 'napm-alert-query') {
          ...
          if (criteria && !criteria.start && !criteria.end) {
            ...
            api.logger.warn('[napm-openclaw-plugin] napm-alert-query: unable to locate criteria in event.params');
          }
        }
        ...
        if (isSafeNapmToolName(toolName) && isPlainObject(event.params)) {
          try {
            ...
            // napm-alert-query: 保留此分支用于 criteria 已有 start/end 但需要
            // timeResolver 做额外校准的场景（如检测毫秒时间戳等）。
            // 默认值兜底已在上方独立逻辑中完成。

...
- `message_received` 还必须生成一份绑定当前 run/message 的不可变 Turn Admission Decision。`DomainIntentClassificationAdapter` 是公共门禁唯一的 prompt-facing 领域分类入口：它只适配和投影既有分类器/判断器，不新增同义正则，并输出带 schemaVersion/source 的不可变结构；`TurnIntentResolver` 只接受这份结构化分类并决定单 Tool 或领域编排，不直接解析 prompt，也不接受临时拼装的 signals。通用选择解析器当前只解析“第 N 个/详情/前 N 条/更换时间”，Query、Alert 等领域解析器只暴露自己的权威结果。公共门禁消费 Decision 中的 route 和 `expectedTool`，不从模型文字复制对象名或内部 ID。

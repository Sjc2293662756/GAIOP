...
- 应用流量范围判断以统一结构化意图为准，不在各层重复猜测关键词。普通查询出现多个 groups 时默认拒绝；只有与静态对象层级验证一致的显式多级下钻路径可以执行。
- 页面族排行后的“查看访问详情/前 N 个访问实例”仍走 `napm-skill-query`，使用独立 `pageViews` 详情服务，不把 `PageFamilyDetail` 伪装成 group。序号选择只能从当前会话冻结的权威 `PageFamily` 结果引用解析；引用无效时说明需要重新查询页面排行，不猜测页面族 ID。

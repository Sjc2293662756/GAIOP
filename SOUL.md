...
查询执行前必须通过共享 ResolvedQueryExecutableValidator：先确认 Metric Catalog 中存在指标，再依据可信产品基线检查 Object × Metric ownership。证据不足时返回“当前能力需要运行时确认”，不调用 metadata、Skill 或南向接口；Gateway、Direct 和 Plugin 不能各自放宽此门禁。

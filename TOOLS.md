...
- `napm-skill-query` Tool adapter 使用 Query Decision Policy 处理必填用户参数；澄清是正常结果，不是 Tool 错误。直接调用 Tool execute 也会执行应用/`TotalTraffic`、`CompositeApplication`/一般对象清单和对象清单单 group 等高风险检查。

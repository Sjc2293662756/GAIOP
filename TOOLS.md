...
- 页面访问实例详情使用 `pageViews/detail`，不使用 `PageFamilyDetail` group。自然语言序号追问传 `resultReference={objectType:"PageFamily",ordinal:N}`；插件从当前 Query Turn 冻结的来源结果集中解析 `pageFamilyId` 并继承时间。引用缺失、过期、跨 scope、类型错误或越界时在 Skill 和南向调用前失败。
